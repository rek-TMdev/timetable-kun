...
時間割枠は平日5日、1日6限の `6行 x 5列` です。`YEARS_MESSAGE` は
`課程`、`YEARS_HIERARCHY` は `全日制` の配下に `2年`、`3年` の葉を
持つ形です。保存キーは `全日制_2年` のように階層名を連結しています。
`HIERARCHY_ANCHORS` は `全日制` に設定しています。枠名は `A1`、`A2`、
`X1` などのスロット ID です。芸術科目セレクト用の候補として `書道Ⅰ`、
`美術Ⅰ`、`音楽Ⅰ` も収録しています。
